...
$[S_{i,j}^t > \kappa] \in \{0,1\}$. If $S_{i,j}^t > \kappa$ is true, $[S_{i,j}^t > \kappa]=1$. If it is fale, $[S_{i,j}^t > \kappa]=0$.\\
$\rho$ is the self-increasing rate of a county if the drug community in that county is larger than 
...
\begin{equation}
 \left(\frac{r2017}{r2010}\right)^{\frac{1}{7}}  
\end{equation}
and take the average. Here, $r2017$ is the number of reports in 2017 while $r2010$ is the number of reports in 2010. This method might not be accurate enough but it is just providing us with a rough range of the actual value $\rho$. In turned out that the average we computed was $1.038$, it follows that we roughly got the value 
\begin{equation}
    \rho = 1.038-1 = 0.038.
\end{equation}
...
\subsection{Part1: Solution Analysis Based on the Inspection of the First Data Set}

Our first model was based on the data provided by the file MCM\_NFLIS\_Data.xlsx. Following is the final spread of drugs yield by our model:



\subsubsection{Drawback}
There are some possible drawbacks of our model. There are drastic changes in drug uses, especially in 2014, the uses of Heroin and Synthetic Opioid increases in a extremely high rate such that we
% labels allow you to cross-reference a section later in the document, without having to remember its number
%  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
% Delete existing text when writing your own report.
